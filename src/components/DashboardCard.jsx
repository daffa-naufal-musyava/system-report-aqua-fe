export default function DashboardCard({
  ...
  // Ensure change is always a string
  const safeChange = typeof change === "string" ? change : "";
  const hasChange = safeChange.length > 0;

  const isUp = safeChange.includes("+");
  const isDown = safeChange.includes("-");
  ...
  return (
    <div className="bg-[#111827]/80 backdrop-blur-sm border border-slate-700/50 rounded-xl p-5 shadow-lg flex flex-col justify-between hover:border-slate-600 transition-colors">
      ...
      <div>
        ...
        {hasChange && (
          <div
            className={`text-xs mt-1 font-medium flex items-center gap-1 ${trendColor}`}
          >
